...
namespace BoletoNet
{
    public abstract class AbstractBanco
    {
        ...
        public virtual void OnGeraHtmlReciboCedente(StringBuilder html, Boleto boleto)
        {
            //não fazer nada nativamente
        }

        /// <summary>
        /// Método que formata o Código do Cedente
        /// </summary>
        /// <remarks>Este método faz uma refatoração da versão da classe BoletoBancario. Ela define um ponto de ajuste descentralizado na implementação</remarks>
        /// <param name="cedente">Variável Cedente</param>
        /// <returns></returns>
        public virtual string FormataAgenciaCodigoCedente(Cedente cedente)
        {
            if (!cedente.DigitoCedente.Equals(-1))
            {
                return string.Format("{0}/{1}-{2}", cedente.ContaBancaria.Agencia, Utils.FormatCode(cedente.Codigo.ToString(), 6), cedente.DigitoCedente.ToString());
            }
            else
            {
                return Utils.FormataAgenciaConta(cedente.ContaBancaria.Agencia, cedente.ContaBancaria.DigitoAgencia, cedente.ContaBancaria.Conta, cedente.ContaBancaria.DigitoConta);
            }
        }
