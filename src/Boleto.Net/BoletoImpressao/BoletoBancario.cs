...
.Replace("@NOSSONUMERO", Boleto.NossoNumero)
        ...
        public byte[] MontaBytesListaBoletosPDF(List<BoletoBancario> boletos, string tituloNaView = "", string CustomSwitches = "", string tituloPDF = "", bool PretoBranco = false, bool convertLinhaDigitavelToImage = false)
        {
            string htmlHead = string.Concat(
                ...
                "</head>",
                "<body>",
                !string.IsNullOrEmpty(tituloPDF) ? "<br/><center><h1>" + tituloPDF + "</h1></center><br/>" : ""
            );
            ...
            return (new NReco.PdfGenerator.HtmlToPdfConverter() { CustomWkHtmlArgs = CustomSwitches, Grayscale = PretoBranco }).GeneratePdf(string.Concat(htmlHead, htmlBoletos, htmlFooter));
        }
